##
## 2021: FIt RCOR models; indeholder både udgave hvor c-kode kaldes og
## udgave med ren R kode; sidstnævnte er dog kommenteret ud.
##
...
## R-based
rcorIPM_r <- function(object, K0, control=object$control, trace=object$trace){
  S   <- dataRep(object, "S")
  n   <- dataRep(object, "n")
  ...
  A         <- refitA_old(S, C.curr, VCC); ## First A estimate
  ...
  while(!converged){
    ...
    C.curr  <- rconIPM(o2, K0=C.curr, control=o2$control, trace=trace - 1, engine="r")$K
    K.curr  <- A * t((A * C.curr)) ## Short for diag(A) %*% C.curr %*% diag(A)
    A       <- refitA_old(S, C.curr, VCC, Astart=A)
    logL    <- ellK(K.curr, S, n - 1);
    dlogL   <- logL - prevlogL

    if (trace>=3)
      cat("...rcorIPM_r iteration", itcount, "logL:", logL, "dlogL:", dlogL, "\n")
...
refitA_old <- function(S, K, NSi, Astart=NULL, itmax=100){
  ...
  repeat{
    ...
    if ((sum( (cprev - cstart)^2) < 1e-9) | it >= itmax){
      ##cat("Iterations:",it,"\n\n")
      break
    }
    cprev <- cstart
  }
  attr(cstart,"iterations") <- it
  return(cstart)
}
...
## rcorIPM_c <- function(object, K0, control=object$control, trace=object$trace){
...
##     if (trace>=2)
##       cat("..rcorIPM_c iteration (offdiag) ", itcount, "logL:", logL, "dlogL:", dlogL, "\n")

##     A       <- refitA(S, C.curr, VCC, Astart=A)
##     ##     KKKK  <- A * t((A*C.curr)) ## Short for diag(A) %*% C.curr %*% diag(A)
##     ##     logLLLL    <- ellK(KKKK,S,n-1);
##     ##     if (trace>=2)
##     ##       cat("..rcorIPM_c iteration (diag)    ", itcount, "logL:", logLLLL, "dlogL:", dlogL, "\n")
