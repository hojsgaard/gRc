...
findKinModel <- function(object, KS, type="rcon", regularize=TRUE)
...
findKinModel.rcox <- function(object, KS, type="rcon", regularize=TRUE){
  ...
  switch(type,
         'rcor'={
           ##C <- cov2cor(KS)
           ##print(C)
           C <- findKinModel(object, cov2cor(KS),   type='rcon')
           A <- findKinModel(object, diag(diag(KS)),type='rcon')
           a <- sqrt(diag(A))
