	     ...
	     SEXP debug)
{
  ...
  int inner=0;       // NOTE 2020: Initializing due to valgrind issue on cran check.
  globalinnerp = 0;  // NOTE 2020: Initializing due to valgrind issue on cran check.
  ...
  for (Gii=0; Gii<nG; Gii++){
    ...
    if (ng > maxng) maxng = ng;
    if (nc > maxnc) maxnc = nc;
    if (ngenElements > maxngenElements) maxngenElements = ngenElements;
  ...
  while(1){
    ...
    for (Gii=0; Gii<nG; Gii++){
      /* if (*debugp >= 2) Rprintf("..Generator %i\n", Gii); */
